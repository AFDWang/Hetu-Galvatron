...
def _run_test(args: Dict[str, Any]):
    ...
    args = RuntimeArgs(model_type=model_type, rank=rank, checkpoint_dir=None, backend=backend)
    ...
    for i, batch in enumerate(trainloader):
        ...
        assert not torch.isnan(loss), f"Loss is NaN in iteration {i}"
...
@pytest.mark.parametrize("backend", ["fa"])
...
# @pytest.mark.parametrize("dp_size", [8])
def test_dp_simple(run_distributed, model_type, backend, dp_size):
