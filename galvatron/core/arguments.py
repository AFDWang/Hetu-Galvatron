...
def galvatron_training_args(parser, use_megatron=True):
    ...
    group.add_argument(
        "--global_tp_deg", type=int, default=-1, help="Global tensor parallel degree.", choices=[-1,1,2,4,8,16,32,64],
    )
    group.add_argument(
    ...
    group.add_argument(
        "--dataset", type=str, default="random", help="indicate which dataset to use, default is random", choices = ['random', 'fix_length', 'common_crawl', 'github', 'wikipedia']
    )
    group.add_argument(
    ...
    group.add_argument(
        "--vocab_tp", type=int, default=1, help="Tensor parallel degree of vocab.",
    )
    group.add_argument(
        "--use-packing", action="store_true", help="Whether to use packing in flash attention",
    )
    group.add_argument(
        "--use-flexSP", action="store_true", help="Whether to use flexSP to enable sequence parallel hot change",
    )
    group.add_argument(
        "--use-ulysses", action="store_true", help="Whether to use DeepSpeed Ulysses or Megatron-TP",
    )
    group.add_argument(
        "--flexSP-strategy", type=str, default="flexSP", help="FlexSP strategy", choices=['flexSP', 'adaptive_bfd', 'fix_sp_bfd'],
    )
