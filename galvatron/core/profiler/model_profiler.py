...
from galvatron.utils.config_utils import read_json_config, write_json_config, num2str, str2array, array2str
import numpy as np
...
class ModelProfiler(BaseProfiler):
    ...
    def set_profiler_launcher(self, path: str, layernum_arg_names: Optional[List[str]] = None, 
                            model_name: Optional[str] = None, seqlen_arg_names: Optional[List[str]] = None, 
                            layernum_listed: bool = False) -> None:
    ...
    def set_layernum_list(self, layernum_arg_names: Optional[List[str]], layernum_listed: bool) -> None:
        ...
        if not self.layernum_listed:
            ...
            self.layernum_list = [layernum for name in layernum_arg_names 
                                for layernum in getattr(self.args, name)]
    ...
    def set_seqlen_list(self) -> None:
        ...
        for i in range(self.num_layertype):
            if args.profile_mode == "static":
                ...
                assert (args.profile_min_batch_size is not None and 
                       args.profile_max_batch_size is not None)
                self.sequence_length_list.append([self.profile_seq_length_list[i]])
                ...
                if args.profile_type == "computation":
                    assert (args.profile_min_seq_length is not None and 
                           args.profile_max_seq_length is not None)
                    self.sequence_length_list.append(
                        list(range(args.profile_min_seq_length, 
                    ...
                    assert ((1<<(args.profile_min_seq_length.bit_length()-1)) == 
                           args.profile_min_seq_length), "profile_min_seq_length must be a power of 2"
                    assert ((1<<(args.profile_max_seq_length.bit_length()-1)) == 
                           args.profile_max_seq_length), "profile_max_seq_length must be a power of 2"
                    self.sequence_length_list.append(
                        [(1<<j) for j in range(args.profile_min_seq_length.bit_length()-1, 
    ...
    def get_bsz_list(self) -> List[int]:
        ...
        if args.profile_mode == "static":
            ...
            assert (args.profile_min_batch_size is not None and 
                   args.profile_max_batch_size is not None)
            self.batch_size_list = list(range(args.profile_min_batch_size, 
    ...
    def launch_profiling_scripts(self) -> None:
        ...
        if args.profile_type == 'memory':
            self._launch_memory_profiling(MODEL_ARGS, PROFILE_ARGS, LAUNCH_SCRIPTS, 
                                        world_size, layernum_lists)
        elif args.profile_type == 'computation':
            self._launch_computation_profiling(MODEL_ARGS, PROFILE_ARGS, LAUNCH_SCRIPTS, 
                                            world_size, layernum_lists)

    def _launch_memory_profiling(self, MODEL_ARGS: str, PROFILE_ARGS: str, 
                               LAUNCH_SCRIPTS: str, world_size: int, 
                               layernum_lists: List[List[int]]) -> None:
        ...
        """
        args = self.args
        assert (args.profile_mode == "static" or 
                args.profile_mode == "sequence"), "Memory profiling only supports sequence or static profile mode."
    ...
    def _launch_computation_profiling(self, MODEL_ARGS: str, PROFILE_ARGS: str, 
                                    LAUNCH_SCRIPTS: str, world_size: int, 
                                    layernum_lists: List[List[int]]) -> None:
    ...
    def process_profiled_data(self) -> None:
        ...
        if args.profile_type == 'computation':
            self._process_computation_data(world_size, layernum_lists)
        elif args.profile_type == 'memory':
            self._process_memory_data(world_size, layernum_lists)

    def _process_computation_data(self, world_size: int, layernum_lists: List[List[int]]) -> None:
    ...
    def _process_memory_data(self, world_size: int, layernum_lists: List[List[int]]) -> None:
        ...
        Note:
            Only supports sequence or static profile modes
        """
        assert (self.args.profile_mode == "static" or 
                self.args.profile_mode == "sequence"), "Memory profiling only support sequence or static profile mode."
        ...
        sequence_length_list = list(product(*self.sequence_length_list))
        for seq in sequence_length_list:
            self._process_single_sequence_config(seq, world_size, layernum_list_base, 
                                              layertype, layernum_lists, layernum_diff,
                                              bsz, config)
    ...
    def _process_single_sequence_config(self, seq: Tuple[int, ...], world_size: int, 
                                      layernum_list_base: List[int], layertype: int,
                                      layernum_lists: List[List[int]], layernum_diff: int,
                                      bsz: int, config: Dict) -> None:
        ...
        while pp_deg * tp_deg <= world_size:
            ...
            if strategy in config:
                ...
                for l in range(layertype):
                    ...
                    param_per_layer = (re[self.key_format(layernum_key_1, bsz, seq_info, 0, 'ms')] - 
                                     re[self.key_format(layernum_key_0, bsz, seq_info, 0, 'ms')]) / layernum_diff * pp_deg / 4
                    
                    # Calculate activation memory per sample
                    act_per_layer_per_sample = (re[self.key_format(layernum_key_1, bsz, seq_info, 0, 'act')] - 
                                              re[self.key_format(layernum_key_0, bsz, seq_info, 0, 'act')]) / layernum_diff * pp_deg / (pp_deg * tp_deg)
        ...
        while pp_deg <= world_size:
            ...
            while pp_deg * tp_deg <= world_size:
                ...
                for enable_vocab_tp in [0, 1]:
                    ...
                    act_peak_first = max(re[self.key_format(layernum_list, bsz, seq_info, 0, 'act_peak')],
                                       re[self.key_format(layernum_list, bsz, seq_info, 0, 'act')])
                    act_peak_last = max(re[self.key_format(layernum_list, bsz, seq_info, world_size-1, 'act_peak')],
                                      re[self.key_format(layernum_list, bsz, seq_info, world_size-1, 'act')])
                    
                    # Calculate other activation memory
                    other_act_first = (act_peak_first - layer_act_costs_first * (bsz / (world_size // (pp_deg * tp_deg)))) / \
                                    (bsz / world_size * pp_deg * (tp_deg if enable_vocab_tp else 1))
                    other_act_last = (act_peak_last - layer_act_costs_last * (bsz / (world_size // (pp_deg * tp_deg)))) / \
                                   (bsz / world_size * pp_deg * (tp_deg if enable_vocab_tp else 1))
    ...
    # =============== Util functions ===============  
    def key_format(self, layernum: Union[List[int], int], bsz: Optional[int] = None, 
                   seq: Optional[Union[str, int]] = None, rank: Optional[int] = None, 
    ...
    def total_memcost(self, pp_deg: int, layernum: int, layertype: int, 
                      per_layer_cost: List[float], stage_idx: int) -> float:
    ...
    def get_layernum_args(self, args: Dict[str, Any], layernum_list: List[int]) -> None:
        ...
        if not self.layernum_listed:
            ...
            assert len(self.layernum_arg_names) == 1, \
                "List format requires exactly one argument name"
