...
def get_dp_group_dict_dist(all_tp_sizes, all_sp_sizes, pp_size, consecutive = False, world_ranks = None):
    tp_sizes_set = list(set(all_tp_sizes))
    sp_sizes_set = list(set(all_sp_sizes))
    for i in range(len(tp_sizes_set)):
        tp_sizes_set[i] = max(tp_sizes_set[i], sp_sizes_set[i])
    dp_group_dict={}
    for tp_size in tp_sizes_set:
...
def gen_redistributed_group(tp_size_old, tp_size_new, tp_consec_old, tp_consec_new, tp_group_old, tp_group_new, 
                                sp_size_old, sp_size_new, sp_group_old, sp_group_new, sequence_parallel = True):
    assert sequence_parallel, "Now only support Megatron with sequence parallel enabled!"
    if (tp_size_old == tp_size_new or sp_size_old == sp_size_new 
        ) and tp_consec_old == tp_consec_new:
        return (None, None)
    tp_group_old = None if tp_size_old == 1 else tp_group_old
    tp_group_new = None if tp_size_new == 1 else tp_group_new
    sp_group_old = None if sp_size_old == 1 else sp_group_old
    sp_group_new = None if sp_size_new == 1 else sp_group_new
    return (tp_group_old if tp_group_old else sp_group_old, tp_group_new if tp_group_new else sp_group_new)
...
# generate seq_data_parallel_group for weight partation
...
def gen_comm_groups(all_tp_sizes, all_sp_sizes, pp_size, tp_consecutive_flags, show_rank = -1, world_ranks = None):
    ...
    for i in range(len(all_tp_sizes)):
        ...
        dp_groups.append(dp_group_dict[1-tp_consecutive_flags[i]][max(all_tp_sizes[i], all_sp_sizes[i])])
        sp_groups.append(sp_group_dict[tp_consecutive_flags[i]][all_sp_sizes[i]])
    for i in range(1, len(all_tp_sizes)):
        split_group, allgather_group = gen_redistributed_group(all_tp_sizes[i-1], all_tp_sizes[i], tp_consecutive_flags[i-1], 
                                                               tp_consecutive_flags[i], tp_groups[i-1], tp_groups[i], 
                                                               all_sp_sizes[i-1], all_sp_sizes[i], sp_groups[i-1], sp_groups[i])
