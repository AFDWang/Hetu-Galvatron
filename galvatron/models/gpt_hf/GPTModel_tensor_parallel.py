...
from megatron.core.tensor_parallel import VocabParallelEmbedding, ColumnParallelLinear, SequenceParallelPositionEmbedding
...
class GPTAttention_tp(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, cu_seqlens = None, max_seqlen = None):
        input_tensor = hidden_states
        hidden_states = self.LayerNorm(hidden_states)
        hidden_states, bias = self.attention(hidden_states, attention_mask, cu_seqlens = cu_seqlens, max_seqlen = max_seqlen)
        if bias is not None:
...
class GPTLayer_tp(nn.Module):
    def __init__(self, config, layer_number, tp_group = None, sp_group = None):
        ...
        self.tp_group = tp_group.group
        self.sp_group = sp_group.group
...
def construct_tensor_parallel_model(model, config, tp_groups_enc, sp_groups_enc, seq_data_group):
    ...
    setattr(model.transformer, 'wte', VocabParallelEmbedding(
            args.padded_vocab_size, megatron_config.hidden_size, config = megatron_config, init_method = megatron_config.init_method, 
            tp_group = tp_groups_enc[0].group, sp_group = sp_groups_enc[0].group))
    if megatron_config.sequence_parallel:
        setattr(model.transformer, 'wpe', SequenceParallelPositionEmbedding(
            config.n_positions, megatron_config.hidden_size, config = megatron_config, init_method = megatron_config.init_method,
                    tp_group = tp_groups_enc[1].group, sp_group = sp_groups_enc[1].group
        ))        
    setattr(model, 'lm_head', ColumnParallelLinear(
            megatron_config.hidden_size, args.padded_vocab_size, config = megatron_config, init_method = megatron_config.init_method, 
            bias=False, tp_group = tp_groups_enc[-1].group, sp_group = sp_groups_enc[-1].group, seq_data_group = seq_data_group.group))
