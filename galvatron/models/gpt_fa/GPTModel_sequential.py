...
class GPTLayers_(nn.Module):
    ...
    def forward(self, hidden_states, position_ids=None, attention_mask=None, labels=None):
        mixer_kwargs = {"seqlen": labels.shape[1]} if self.process_group is not None and self.sequence_parallel else {}
        if self.prenorm:
            assert not self.parallel_block
            hidden_states, residual = self.layer(hidden_states, mixer_kwargs=mixer_kwargs)
