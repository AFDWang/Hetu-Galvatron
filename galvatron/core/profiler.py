...
class GalvatronProfiler():
    ...
    def profile_time_start(self, iter):
        ...
        if self.print_avg:
            ...
            if self.exit and iter == self.end_iter:
                exit(0)
    ...
    def profile_time_end(self, iter):
        ...
        if iter >= self.start_iter and iter < self.end_iter:
            ...
            self.cur_iter_time += iter_time
            # self.time_list.append(iter_time)
            if torch.distributed.get_rank() % 8 == 0:
                print(f'[Micro Batch Iter {iter} time: {iter_time:.4f}]')
                
    def profile_time_iter_end(self, iter):
        if not self.args.profile:
            return
        if iter >= self.start_iter and iter < self.end_iter:
            self.time_list.append(self.cur_iter_time)
            self.print_avg = True
            if torch.distributed.get_rank() %8 ==0:
                print(f'[GLOBAL BATCH Iter {iter} time: {self.cur_iter_time:.4f}]')    
