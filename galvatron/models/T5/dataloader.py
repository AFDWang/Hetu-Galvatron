...
from megatron.core.datasets.t5_dataset import T5MaskedWordPieceDatasetConfig, T5MaskedWordPieceDataset
...
def random_collate_fn(batch):
    ...
    return enc_tokens, {"dec_tokens":dec_tokens, "enc_attn_mask":enc_mask, "dec_attn_mask":dec_mask, "enc_dec_attn_mask":enc_dec_mask, "dec_labels" : dec_labels}, None

def _make_attention_mask(
    source_block: np.ndarray, target_block: np.ndarray
) -> np.ndarray:
    ...
    def __init__(self, args, device, dataset_size = 20 * 16):
        ...
        if os.path.exists(cache_file):
            ...
            for i in range(self.dataset_size):
                ...
                self.dec_mask.append(_make_attention_mask(dec_sentence, dec_sentence) * _make_history_mask(dec_sentence))
