        ...
        if 'enc' in module_type or 'dec' in module_type:
            ...
            if tied_wte_attr_names is not None:
                if module_type == 'embed':
                    for _ in range(len(tied_wte_attr_names) - 1):
                        assert rhasattr(module.module, tied_wte_attr_names[_])
                        tied_module = rgetattr(module.module, tied_wte_attr_names[_])
                        rsetattr(module.module, tied_wte_attr_names[_], FSDP(tied_module, **fsdp_args))
