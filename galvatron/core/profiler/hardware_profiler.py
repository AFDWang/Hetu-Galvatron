...
class HardwareProfiler(BaseProfiler):
        ...
        while allreduce_size > 1:
            for is_consecutive in [True, False]:
                if world_size == allreduce_size and not is_consecutive:
                    continue
                print('============= allreduce_size: %d, allreduce_consec: %d ============='%(allreduce_size, is_consecutive))
    ...
    def generate_allreduce_groups(self, world_size: int, allreduce_size: int, allreduce_consec: bool) -> List[List[int]]:
    ...
    def launch_nccl_test(self, groups: List[List[int]], num_gpus_per_node: int, ARGS: str, mode: str = 'avg') -> Union[float, Tuple[List[int], List[float]]]:
