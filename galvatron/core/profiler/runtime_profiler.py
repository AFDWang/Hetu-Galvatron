...
class RuntimeProfiler(BaseProfiler):
    ...
    def set_profiler_dist(self, path: Optional[str] = None, 
        ...
        if args.profile and iter == max_profile_iter:
            if rank in profile_ranks:
                ...
                if not hasattr(args, 'pipeline_type') or args.pipeline_type == "gpipe":
                    ...
                    mem_dict['activation'] = (mem_dict[f'iter_{max_profile_iter-1}_after_forward'] - 
                ...
                mem_dict['peak_activation'] = (mem_dict[f'iter_{max_profile_iter-1}_after_backward_max'] - 
    ...
    def profile_time_end(self, iter: int, loss: Optional[torch.Tensor] = None, 
    ...
    def profile_time_python(self, iter: int) -> None:
        ...
        if iter == self.start_iter:
            ...
            if hasattr(args, 'profile_forward') and args.profile_forward:
                ...
                save_profiled_time(time_config_path, avg_time, args.global_train_batch_size, 
                                 self.layernum_list, self.seqlen_list)
    ...
    def _process_time_results(self) -> None:
        ...
        if hasattr(args, 'profile_forward') and args.profile_forward:
            assert self.layernum_list is not None
            time_config_path = self.time_profiling_path()
            save_profiled_time(time_config_path, avg_time*1e3, args.global_train_batch_size,
                             self.layernum_list, self.seqlen_list)
    ...
    def _log_iteration_stats(self, iter: int, iter_time: float, loss: Optional[torch.Tensor],
                            learning_rate: Optional[float], grad_norm: Optional[float]) -> None:
