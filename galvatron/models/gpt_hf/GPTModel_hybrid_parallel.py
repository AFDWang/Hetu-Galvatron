...
def construct_hybrid_parallel_model(model, model_config, training_args, hybrid_parallel_configs):
    wrap_block_name = [GPTLayer_tp, LayerNorm] if not training_args.use_ulysses else [GPTLayer_tp]
    hp_model = construct_hybrid_parallel_model_api(
        ...
        tied_wte_attr_names=['wte', "wps", 'lm_head'],
        sp_layernorm_attr_names=['layer.attention.LayerNorm', 'layer.mlp.LayerNorm'] if not training_args.use_ulysses else None
    )
    return hp_model
