...
class CoreAttention(MegatronModule):
    ...
    def __init__(self, layer_number, config,
        ...
        seq_parallel_world_size = 1 if sp_group == None else torch.distributed.get_world_size(sp_group)
        tp_world_size = 1
        if tp_group is None:
            tp_world_size = mpu.get_tensor_model_parallel_world_size()
        else:
            tp_world_size = tensor_parallel.get_tensor_model_parallel_world_size_group(tp_group)
        world_size = max(seq_parallel_world_size, tp_world_size)
...
class FlashSelfAttention(torch.nn.Module):
    ...
    def forward(self, q, k, v, cu_seqlens = None, max_seqlen = None):
        ...
        if use_packing:
            cu_seqlens_k = cu_seqlens_q
            #we target training, so set to max_seqlen directly
...
class DistributedAttention(torch.nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, batch_dim_idx: int, *args: Any, **kwargs) -> Tensor:
        ...
        #out shape : e.g., [s:h/p:]
        if get_args().use_packing: #[1,S,H,D] -> [S,H,D]
            query_layer, key_layer, value_layer = [x.squeeze(0) for x in (query_layer, key_layer, value_layer)]
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, config, layer_number,
        ...
        # world_size = max(sp_world_size, tp_world_size)
        self.hidden_size_per_attention_head = core.utils.divide(
        ...
        if self.use_flash_attn:
            ...
            local_attn = CoreAttention(self.layer_number, config,
                                            self.attn_mask_type, tp_group=tp_group, sp_group = sp_group)
        
        if args.use_ulysses:
            assert args.num_attention_heads % sp_world_size == 0
            self.dist_attn = DistributedAttention(local_attn, sp_group, 
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        use_packing = get_args().use_packing
        if use_packing:
            assert self.use_flash_attn, "packing is only supported when flash-attn used"
            assert cu_seqlens is not None, "cu_seqlen must be given!"
            assert max_seqlen, "max_seqlen must be given!"
        args = get_args()
        if args.use_ulysses:
            ...
            if self.use_flash_attn:
                ...
                if args.use_flexSP:
                    self.dist_attn.spg = args.sp_group
                context_layer = self.dist_attn(query_layer, key_layer, value_layer, batch_dim_idx, cu_seqlens, max_seqlen)
            ...
            if not self.use_flash_attn:
                ...
                if not self.sequence_parallel:
                    with tensor_parallel.get_cuda_rng_tracker().fork():
                        context_layer = self.core_attention_flash(q, k, v, cu_seqlens, max_seqlen)
                else:
                    context_layer = self.core_attention_flash(q, k, v, cu_seqlens, max_seqlen)
                context_layer = rearrange(context_layer, 'b s h d -> s b (h d)').contiguous()
